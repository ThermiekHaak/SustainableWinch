...
from FuelcellSystem import FuelCellSystem
from H2Tank import H2Tank
...
class Powertrain(Base):
    ...
    def power_calc(self):
        ...
        if self.energy_source == 'BEV':
            ...
            overnight_charging_energy = self.overnight_charging_power*self.overnight_charging_time*3600
            BAT_capacity = energy_per_day*self.consecutive_operation-(overnight_charging_energy*self.consecutive_operation-1)
            BAT_power = max(self.p_profile)/1e3
            ...
            power_ratio = 0.5
            FC_power = power_ratio*max(self.p_profile)/1e3
